...
// solving the fields.  the code moves only electronss, with periodic
...
// default unit numbers
      ...
      double[] wpmax = new double[1]; double[] wpmin = new double[1];
      double totpush = 0.0;
      double qbme, affp, qi0, omt, q2m0, wp0;
      double[] part, qe, fxe, cu, cus, amu, fxyze, eyze, byze;
      double[] pt, sfield, vfield, fv, fvm, wt;
      double[] ffc, ffe, sct, pott, vpott;
      int[] mixup, ip, npic;
...
// amu(i,j) = i component of momentum flux at grid (j).
      cu = new double[ndim*nxe]; cus = new double[ndim*nxe];
      amu = new double[ndim*nxe];
...
// over the particle shape
      byze = new double[ndim*nxe]; eyze = new double[ndim*nxe];
// ffc, ffe = form factor arrays for poisson solvers
      ffc = new double[2*nxh]; ffe = new double[2*nxeh];
...
// vector potential diagnostic
      if (nta > 0) {
         vfield = new double[ndim*nxe];
         if (modesxa > nxh)
            modesxa = nxh;
         vpott = new double[2*ndim*modesxa];
// open output file
         if (narec[0]==0) {
         ...
         vfield = new double[0]; vpott = new double[0];
      }
// record time
      jtimer(tloop,ltime,1);
      time = tloop[0];
...
// deposit current and momentum flux for electrons
         djpost(part,cu,np,qme,zero,tdjpost,nx,ipbc,idimp,ndim,inorder,
                djopt);
         dmjpost(part,amu,np,qme,tdcjpost,idimp,ndim,inorder,djopt);
// initialize charge density to background
...
// find maximum and minimum plasma frequency
         wpmxn(qe,qi0,qbme,wpmax,wpmin,nx,inorder);
         wp0 = 0.5*(wpmax[0] + wpmin[0]);
// recalculate form factors
         if ((wp0 > 1.15*q2m0) || (wp0 < 0.85*q2m0)) {
            q2m0 = wp0;
            wp0 = affp*wp0;
            epois_init(ffe,ax,affp,wp0,ci,nx);
            System.out.println(ntime+" new shift constants,q2m0,wp0="+
                               q2m0+""+wp0);
         }
// velocity diagnostic
         if (ntv > 0) {
...
// calculate magnetic field in fourier space
         bpois(cu,byze,ffc,ci,wm,nx,ndim,inorder);
...
// transform acceleration density and momentum flux to fourier space
            isign = -1;
            fft2(cus,isign,mixup,sct,tfft,indx,ndim,inorder);
            fft2(amu,isign,mixup,sct,tfft,indx,ndim,inorder);
// take time derivative of current
            adcuperp(cus,amu,nx,ndim,inorder);
...
// calculate vector potential in fourier space
               apois(cu,vfield,ffc,ci,wm,nx,ndim,inorder);
// store selected fourier modes
...
// push particles
         wke[0] = 0.0;
         push3(part,fxyze,byze,omx,np,qbme,dt,dt,wke,tpush,nx,ipbc,
...
// accumulate timings
      jot.printf("darwin Java code dbeps1%n");
      jot.printf("main wall clock time = %f sec%n",time);
      totpush = tpush[0] + tdpost[0] + tdjpost[0] + tdcjpost[0];
      ...
      jot.printf("electron acceleration deposit time = %f sec%n",
                 tdcjpost[0]);
